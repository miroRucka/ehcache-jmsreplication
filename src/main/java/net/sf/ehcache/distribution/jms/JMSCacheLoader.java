...
public class JMSCacheLoader implements CacheLoader {
	...
	private static final Logger LOG = Logger.getLogger(JMSCacheLoader.class
			.getName());
	...
	public JMSCacheLoader(Ehcache cache, String defaultLoaderArgument,
			...
			AcknowledgementMode acknowledgementMode, int timeoutMillis) {
	...
	/**
	 * loads an object. Application writers should implement this method to
	 * customize the loading of cache object. This method is called by the
	 * caching service when the requested object is not in the cache.
	 * <p/>
	 * 
	 * @param key
	 ...
	 * <p/>
	 * JCache will call through to the load(key) method, rather than this
	 * method, where the argument is null.
	 * 
	 * @param key
	 *            the key to load the object for.
	 * @param argument
	 *            can be anything that makes sense to the loader. The argument
	 *            is converted to a String with toString() to use for the JMS
	 *            StringProperty loaderArgument
	 ...
	 */
	protected Object loadFromJMS(JMSEventMessage jmsEventMessage)
			throws CacheException {
		Object value;
		...
		try {
			...
			ObjectMessage loadRequest = getQueueSession
					.createObjectMessage(jmsEventMessage);
			...
			loadRequest.setIntProperty(CACHE_MANAGER_UID,
					localCacheManagerUid(cache));
			getQueueSender.send(loadRequest, DeliveryMode.NON_PERSISTENT,
					HIGHEST_JMS_PRORITY, timeoutMillis);

			// must send first before getting id
			String initialMessageId = loadRequest.getJMSMessageID();

			ObjectMessage reply = (ObjectMessage) replyReceiver
			...
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("Initial ID: " + initialMessageId
						+ ". Reply Correlation ID. " + messageId);
			}
			...
			try {
				...
				LOG.log(Level.SEVERE,
						"Problem closing JMS Resources: " + e.getMessage(), e);
			}
		}
		return value;
	}
	...
	/**
	 * loads multiple object. Application writers should implement this method
	 * to customize the loading of cache object. This method is called by the
	 * caching service when the requested object is not in the cache.
	 * <p/>
	 * 
	 * @param keys
	 ...
	 * 
	 * @param keys
	 *            a <code>Collection</code> of keys to load objects for. Each
	 *            key must be <code>Serializable</code>.
	 * @param argument
	 *            can be anything that makes sense to the loader. It must be
	...
	public String getName() {
		return "JMSCacheLoader with default loaderArgument: "
	 ...
	 * changed it's status to alive. Cache operations are legal in this method.
	 * 
		...
		try {
			getQueueConnection.setExceptionListener(new ExceptionListener() {
				...
				public void onException(JMSException e) {
					LOG.log(Level.SEVERE, "Exception on getQueue Connection: "
							+ e.getMessage(), e);
				}
			});
	...
	public void dispose() throws net.sf.ehcache.CacheException {
		JMSUtil.closeMessageProducer(getQueueSender);
		JMSUtil.closeSession(getQueueSession);
		JMSUtil.closeConnection(getQueueConnection, true);
